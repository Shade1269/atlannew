-- Reverse sync trigger: order_hub → ecommerce_orders
-- This ensures admin status updates trigger commission/revenue distribution
...
AS $$
BEGIN
  -- Only sync if source_id exists (linked to ecommerce_orders)
...
-- Create the trigger on order_hub for status/payment_status updates
...
COMMENT ON FUNCTION public.sync_ecommerce_from_order_hub() IS 
'Reverse sync from order_hub to ecommerce_orders. When admin updates order status in order_hub, this trigger propagates changes to ecommerce_orders, which then triggers commission generation and revenue distribution.';