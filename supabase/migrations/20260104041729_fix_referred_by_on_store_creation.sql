-- ============================================
-- إصلاح تعيين referred_by عند إنشاء المتجر
-- Fix referred_by assignment when creating affiliate store
-- ============================================

-- تحديث دالة create_affiliate_store لضمان تعيين referred_by
-- Update create_affiliate_store function to ensure referred_by is set
...
BEGIN
  ...
  -- ملاحظة: الـ trigger link_store_to_referrer سيعمل أيضاً، لكن هذا يضمن التعيين الصحيح
  ...
  -- Verify referred_by was set correctly (double-check after trigger)
  IF v_referred_by IS NOT NULL THEN
    -- Double-check and update if needed (in case trigger didn't work)
...
-- التأكد من أن trigger link_store_to_referrer موجود ويعمل
-- Ensure trigger link_store_to_referrer exists and works
...
-- إصلاح المتاجر الموجودة التي لم يتم تعيين referred_by لها
-- Fix existing stores that should have referred_by but don't
DO $$
DECLARE
