-- إصلاح مشكلة الغموض في عمود status
...
BEGIN
  ...
  SELECT 
    ...
    CASE 
      WHEN (SELECT COUNT(*) FROM refunds r WHERE r.status = 'PENDING') > 5 THEN 'warning'::TEXT
      ELSE 'success'::TEXT
    END AS check_status,
    jsonb_build_object(
      'total_refunds', (SELECT COUNT(*) FROM refunds),
      'pending', (SELECT COUNT(*) FROM refunds r WHERE r.status = 'PENDING'),
      'approved', (SELECT COUNT(*) FROM refunds r WHERE r.status = 'APPROVED'),
      'completed', (SELECT COUNT(*) FROM refunds r WHERE r.status = 'COMPLETED')
    ) AS details;
  ...
  SELECT 
    ...
    CASE 
      WHEN (SELECT COUNT(*) FROM shipments sh WHERE sh.status = 'PENDING') > 10 THEN 'warning'::TEXT
      ELSE 'success'::TEXT
    END AS check_status,
    jsonb_build_object(
      'total_shipments', (SELECT COUNT(*) FROM shipments),
      'pending', (SELECT COUNT(*) FROM shipments sh WHERE sh.status = 'PENDING'),
      'in_transit', (SELECT COUNT(*) FROM shipments sh WHERE sh.status = 'IN_TRANSIT'),
      'delivered', (SELECT COUNT(*) FROM shipments sh WHERE sh.status = 'DELIVERED')
    ) AS details;
